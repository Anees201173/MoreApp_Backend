...
const createCompany = asyncHandler(async (req, res) => {
    ...
    // Check if companyAdmin already has a company
    // const existingCompany = await Company.findByCompanyAdmin(companyAdmin);
    // if (existingCompany) {
    //     throw new ApiError(400, 'This user is already admin of another company');
    // }
...
const getAllCompanies = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(
        new ApiResponse(200, result, 'Companies retrieved successfully')
    );
});
...
const toggleCompanyStatus = asyncHandler(async (req, res) => {
    ...
    const sanitizedCompany = sanitizeObject(company.toJSON(), [
        'password'
    ]);
